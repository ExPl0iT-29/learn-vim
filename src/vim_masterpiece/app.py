...
from vim_masterpiece.audio import AudioEngine
from vim_masterpiece.core.engine import GameEngine
from vim_masterpiece.core.vim_logic import VimParser
from vim_masterpiece.core.config import ConfigManager
from vim_masterpiece.data.levels import LEVELS
from vim_masterpiece.data.models import GameMode
from vim_masterpiece.data.narrative import MILESTONE_1, MILESTONE_10, MILESTONE_30
from vim_masterpiece.ui.widgets import VimMap, StatsDisplay, CommandBar, SoundBubble, NarrativeOverlay, HelpPanel, HelpOverlay
from vim_masterpiece.mechanics.engines import ScoringEngine
...
class VimMasterpiece(App):
    ...
    def on_mount(self) -> None:
        ...
        self.audio = AudioEngine(self, sound_enabled=self.config.get("sound_enabled"))
    ...
    def load_level(self):
        ...
        if level_index < len(LEVELS):
            if level_index == 0:
                self.show_narrative(MILESTONE_1)
            elif level_index == 9:
                self.show_narrative(MILESTONE_10)
            elif level_index == 29:
    ...
    def show_narrative(self, content: str):
        self.query_one("#narrative").show(content)
    ...
    def on_key(self, event) -> None:
        ...
        # textual sends "colon" / "question_mark" for : and ? — use character instead
        key = event.character if (event.character and event.character.isprintable()) else event.key
        ...
        if self.config.get("sound_enabled"):
            bubble.display("CLACK" if key.isalnum() else "TICK")

        buffer = self.parser.handle_key(key, self.engine.mode)
