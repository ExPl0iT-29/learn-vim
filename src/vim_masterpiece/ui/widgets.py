...
from vim_masterpiece.data.models import Point, GameMode
        ...
        for row_index, row in enumerate(map_data):
            ...
            for col_index, char in enumerate(row):
                ...
                if is_player:
                    ...
                    row_str += f"[{style_prefix}bold #f7768e]G[/]"
                elif char == "B":
                    row_str += f"[{style_prefix}bold italic #f7768e on #1f2335]B[/]"
                elif char == ">":
                    row_str += f"[{style_prefix}bold #9ece6a]>[/]"
                elif char == "R":
                    row_str += f"[{style_prefix}#565f89]R[/]"
                elif char.islower():
                    row_str += f"[{style_prefix}bold #e0af68]{char}[/]"
                elif char.isupper():
                    row_str += f"[{style_prefix}bold #bb9af7]{char}[/]"
                else:
                    if is_selected:
                        row_str += f"[{style_prefix}#c0caf5]. [/]"
                    else:
                        row_str += "[#1a1b26]. [/]"
            rows.append(row_str)
        self.update("\n".join(rows))
